...
famulor.de offers multiple ways to handle phone numbers for inbound or outbound calls. You can **rent** a dedicated number from us, use a **SIP** trunk (no monthly fee but carrier costs are on your own PBX), or verify an **existing Caller ID** (also no monthly fee, but region-based per-minute charges).
...
- **Monthly Fee**: Typically €3.99/month for standard local numbers (US/CA/UK). Regions vary:
  - **Europe**: Often €2-€8/month based on country.
  - **Latin America, Asia, Africa**: €9+ in some cases, depending on availability or local taxes
...
- Outbound calls on a rented number: depends on the country you’re calling—could range from €0.05/min to €0.45/min if it’s an expensive destination.
...
If you already have a VOIP/PBX solution, you can integrate via a **SIP trunk** with **no monthly rental** from famulor.de. Instead, you pay:

- **SIP Per-Minute**: €0.0004/min on our side for carrier bridging. Your VOIP provider may also have separate usage fees.
- **Your Existing Number**: You’ll continue to use your own phone lines for inbound and outbound, but the carrier layer costs €0.0004/min with us whenever the AI is on the call.
...
- **Carrier Rates**: Some countries cost €0.01/min, others €0.45/min due to local carriers.
...
2. **Use SIP**
   - No monthly from us, but a €0.0004/min AI bridging cost, plus your own PBX fees.
3. **Caller ID**
