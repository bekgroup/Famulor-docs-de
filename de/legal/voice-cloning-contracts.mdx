---
title: "Voice Cloning Vertragsvorlagen"
description: "Musterverträge und Einwilligungserklärungen für Voice Cloning in Deutschland, Österreich und der Schweiz"
icon: "file-contract"
---


# Voice Cloning Vertragsvorlagen

## 8. Praktische Umsetzung

### 8.1 Einholung und Dokumentation von Einwilligungen

#### Rechtliche Grundlage

In Deutschland, Österreich und der Schweiz ist die **Stimme als personenbezogenes und schützenswertes Merkmal** anerkannt. Voice Cloning ohne ausdrückliche Zustimmung kann erhebliche rechtliche Konsequenzen haben.

<Warning>
Die Einwilligung ist **zwingend erforderlich**, um Voice Cloning rechtssicher einzusetzen.
</Warning>

#### Anforderungen an die Einwilligung

<Steps>
  <Step title="Freiwilligkeit">
    Einwilligung muss freiwillig, informiert und ausdrücklich erfolgen
  </Step>
  <Step title="Dokumentation">
    Schriftliche oder elektronische Dokumentation erforderlich
  </Step>
  <Step title="Zweckbindung">
    Klare Definition von Zweck, Umfang und Dauer der Nutzung
  </Step>
  <Step title="Widerrufbarkeit">
    Jederzeit widerrufbar mit klaren Folgen
  </Step>
</Steps>

#### Best Practices

<CardGroup cols={2}>
  <Card title="Transparenz" icon="eye">
    Klare Information über Zweck und Umfang der Stimmnutzung
  </Card>
  <Card title="Nachweisführung" icon="file-text">
    Sorgfältige Archivierung und Schutz vor unbefugtem Zugriff
  </Card>
  <Card title="Minderjährigenschutz" icon="child">
    Einwilligung der Erziehungsberechtigten erforderlich
  </Card>
  <Card title="Kennzeichnung" icon="tag">
    Erkennbarkeit für Nutzer und Dritte bei synthetischen Stimmen
  </Card>
</CardGroup>

---

## Musterverträge

### Deutschland: Einwilligung und Nutzungsvereinbarung

<Accordion title="Vollständiger Mustervertrag für Deutschland">

#### Einwilligung und Nutzungsvereinbarung für Voice Cloning (Deutschland)

**Vertragsparteien**

**Sprecher:in:**
- Name: ______________________________
- Adresse: ___________________________

**Auftraggeber:in:**
- Name/Firma: _______________________
- Adresse: ___________________________

---

**1. Vertragsgegenstand**

Gegenstand dieses Vertrags ist die Nutzung der Stimme des/der Sprecher:in für die Produktion von Sprachaufnahmen und deren Verwendung im Rahmen von Voice Cloning und künstlicher Intelligenz.

**2. Umfang der Rechteübertragung**

Die Nutzung der Stimme, ihrer Modulation, Klangfarbe, der damit verbundenen Gestik sowie aller vergleichbaren Merkmale des/der Sprecher:in ist **ausschließlich für den im vorliegenden Vertrag genau definierten Zweck** und die konkrete Produktion erlaubt.

<Warning>
Die Verwendung der Stimme zur Einspeisung (einschließlich Text- und Data-Mining), Archivierung, zum Training, zur Simulation oder für sonstige Aktivitäten im Rahmen von KI, maschinellem Lernen oder Voice Cloning ist **ohne ausdrückliche schriftliche Zustimmung NICHT gestattet**.
</Warning>

**3. Einwilligung und Vergütung**

- Die Nutzung der Stimme für KI-Anwendungen (einschließlich Voice Cloning) bedarf einer **ausdrücklichen, schriftlichen Einwilligung**
- Für die Nutzung im Rahmen von KI-Anwendungen ist eine **gesonderte, angemessene Vergütung** zu vereinbaren
- Vergütung: ________________________ €

**4. Datenschutz (DSGVO-konform)**

Die Verarbeitung der Sprachaufnahmen erfolgt gemäß DSGVO. Die Stimme gilt als **personenbezogenes biometrisches Datum** und darf nur mit Einwilligung verarbeitet werden.

**Rechte des/der Sprecher:in:**
- Auskunftsrecht über gespeicherte Daten
- Löschungsrecht ("Recht auf Vergessenwerden")
- Widerspruchsrecht gegen Datenverarbeitung

**5. Widerruf und Löschung**

Die Einwilligung zur Nutzung der Stimme für KI-Anwendungen kann **jederzeit mit Wirkung für die Zukunft widerrufen** werden. 

Im Falle des Widerrufs verpflichtet sich der/die Auftraggeber:in:
- Unverzügliche Löschung aller gespeicherten Sprachdaten
- Löschung aller daraus abgeleiteten KI-Modelle
- Einstellung jeder weiteren Nutzung

**6. Zweckbindung**

Verwendungszweck: ___________________

Die Nutzung ist **strikt zweckgebunden** und darf nicht für andere Zwecke verwendet werden.

**7. Schlussbestimmungen**

- Änderungen bedürfen der **Schriftform**
- Bei Unwirksamkeit einzelner Bestimmungen bleibt der Rest des Vertrags wirksam
- Es gilt **deutsches Recht**

---

**Ort, Datum:** ______________________

**Unterschrift Sprecher:in:** ______________________

**Unterschrift Auftraggeber:in:** ______________________

</Accordion>

### Österreich: DSGVO-konforme Vorlage

<Accordion title="Mustervertrag für Österreich">

#### Einwilligung und Nutzungsvereinbarung für Voice Cloning (Österreich)

**zwischen**

**Anbieter:** [Name und Adresse des Unternehmens]

**und**

**Nutzer/in:** [Name und Adresse der betroffenen Person]

---

**1. Gegenstand der Vereinbarung**

Der/die Nutzer/in erklärt sich damit einverstanden, dass der Anbieter die Stimme mittels **technischer Verfahren (Voice Cloning)** aufzeichnet, verarbeitet und ein KI-Modell erstellt, das die Stimme simuliert. 

Die Verarbeitung erfolgt gemäß **DSGVO** und dem **österreichischen Datenschutzgesetz**.

**2. Zweck der Nutzung**

Die geklonte Stimme wird ausschließlich zu folgenden Zwecken verwendet:

☐ Training von KI-Systemen  
☐ Erstellung von Sprachaufnahmen für: _________________  
☐ Weitere: ____________________________

**3. Art und Umfang der Nutzung**

- Nutzung **ausschließlich** zu den oben genannten Zwecken
- Weitergabe an Dritte nur mit **ausdrücklicher Zustimmung**
- Kommerzielle Nutzung: ☐ Ja ☐ Nein

**4. Speicherdauer**

- Stimmaufnahmen und KI-Modell werden für ____________ gespeichert
- Nach Ablauf erfolgt **Löschung**, sofern keine gesetzlichen Aufbewahrungspflichten bestehen

**5. Widerrufsrecht**

<Info>
Der/die Nutzer/in kann diese Einwilligung **jederzeit ohne Angabe von Gründen** mit Wirkung für die Zukunft widerrufen.
</Info>

**Folgen des Widerrufs:**
- Unverzügliche Löschung der Stimmaufnahmen
- Löschung des KI-Modells
- Einstellung aller weiteren Verarbeitungen

**6. DSGVO-Rechte**

Der/die Nutzer/in hat folgende Rechte:
- **Auskunftsrecht** (Art. 15 DSGVO)
- **Löschungsrecht** (Art. 17 DSGVO)
- **Berichtigungsrecht** (Art. 16 DSGVO)
- **Datenübertragbarkeit** (Art. 20 DSGVO)

**7. Datensicherheit**

Der Anbieter verpflichtet sich zu **geeigneten technischen und organisatorischen Maßnahmen** zum Schutz der Daten.

**8. Haftung**

- Haftung nur bei **Vorsatz oder grober Fahrlässigkeit**
- Für Schäden durch unbefugte Drittnutzung nur bei nachweisbarer Sorgfaltspflichtverletzung

**9. Anwendbares Recht**

Es gilt **österreichisches Recht**. Gerichtsstand ist [Ort].

---

**Ort, Datum:** ____________________

**Unterschrift Anbieter:** ____________________

**Unterschrift Nutzer/in:** ____________________

</Accordion>

### Schweiz: Datenschutzkonform

<Accordion title="Mustervertrag für die Schweiz">

#### Einwilligung und Nutzungsvereinbarung für Voice Cloning (Schweiz)

**zwischen**

**Verantwortliche Stelle:** [Name und Adresse]

**und**

**Betroffene Person:** [Name und Adresse]

---

**1. Vertragsgegenstand**

Die betroffene Person erklärt sich damit einverstanden, dass die verantwortliche Stelle die Stimme mittels **Voice Cloning-Technologie** aufzeichnet, verarbeitet und ein KI-Modell zur Stimmimitation erstellt.

**2. Rechtliche Grundlage**

Die Verarbeitung erfolgt gemäß:
- **Schweizer Datenschutzgesetz (DSG)**
- **Bundesgesetz über den Datenschutz (FADP)**
- EU-Adequacy Decision für grenzüberschreitende Verarbeitung

**3. Verwendungszwecke**

☐ KI-System Training  
☐ Sprachaufnahmen für: _______________  
☐ Weitere Zwecke: ___________________

**4. Verarbeitungsumfang**

- **Ausschließliche Nutzung** für definierte Zwecke
- Drittübertragung nur mit **expliziter Genehmigung**
- **Kommerzielle Verwertung:** ☐ Ja ☐ Nein

**5. Datenaufbewahrung**

- Aufbewahrungsdauer: ________________
- **Automatische Löschung** nach Ablauf
- Ausnahme: Gesetzliche Aufbewahrungspflichten

**6. Widerrufsrecht**

<Warning>
**Jederzeitiges Widerrufsrecht** ohne Begründung mit sofortiger Wirkung für die Zukunft.
</Warning>

**Widerrufsfolgen:**
- Sofortige Einstellung der Verarbeitung
- Löschung aller Stimmaufnahmen
- Vernichtung des KI-Modells

**7. Betroffenenrechte nach DSG**

- **Auskunftsrecht** über verarbeitete Daten
- **Berichtigungsrecht** bei falschen Angaben
- **Löschungsrecht** bei rechtswidriger Verarbeitung
- **Widerspruchsrecht** gegen Verarbeitung

**8. Datensicherheit**

Verpflichtung zu **angemessenen technischen und organisatorischen Schutzmaßnahmen** entsprechend dem Stand der Technik.

**9. Grenzüberschreitende Datenübertragung**

- Transfer nur in **Länder mit angemessenem Datenschutzniveau**
- EU-Länder durch Adequacy Decision abgedeckt
- Drittstaaten nur mit zusätzlichen Garantien

**10. Haftungsregelung**

- Haftung bei **vorsätzlicher oder grobfahrlässiger** Schädigung
- Ausschluss der Haftung für **höhere Gewalt**
- Versicherungsschutz für Datenschutzverletzungen

**11. Anwendbares Recht**

**Schweizer Recht** ist anwendbar. Zuständiges Gericht: [Ort].

---

**Ort, Datum:** ____________________

**Unterschrift Verantwortliche Stelle:** ____________________

**Unterschrift Betroffene Person:** ____________________

</Accordion>

---

## 8.2 Lizenzmodelle und Kommerzialisierung

### Lizenzarten

<CardGroup cols={2}>
  <Card title="Exklusive Lizenz" icon="crown">
    Nur der Lizenznehmer darf die Stimme für vereinbarte Zwecke nutzen
  </Card>
  <Card title="Nicht-exklusive Lizenz" icon="users">
    Mehrere Lizenznehmer können die Stimme gleichzeitig nutzen
  </Card>
  <Card title="Zeitlich begrenzt" icon="clock">
    Nutzung auf bestimmten Zeitraum beschränkt
  </Card>
  <Card title="Zweckgebunden" icon="bullseye">
    Verwendung nur für spezifische Anwendungen erlaubt
  </Card>
</CardGroup>

### Vergütungsmodelle

<Tabs>
  <Tab title="Einmalzahlung">
    **Pauschale Abgeltung:**
    - Feste Summe für definierte Nutzungsrechte
    - Einfache Abwicklung
    - Kalkulierbare Kosten für Lizenznehmer
  </Tab>
  
  <Tab title="Laufende Royalties">
    **Umsatzbeteiligung:**
    - Prozentuale Beteiligung am Umsatz
    - Langfristige Einnahmen für Stimmgeber
    - Erfolgsabhängige Vergütung
  </Tab>
  
  <Tab title="Hybridmodelle">
    **Kombinierte Ansätze:**
    - Grundvergütung plus Erfolgsbeteiligung
    - Staffelpreise nach Nutzungsumfang
    - Mengenrabatte bei Mehrfachlizenzen
  </Tab>
</Tabs>

### 8.3 Risiken bei unautorisierter Nutzung

#### Rechtliche Risiken

<Warning>
Unautorisierte Stimmnutzung kann zu schwerwiegenden rechtlichen Konsequenzen führen.
</Warning>

<CardGroup cols={2}>
  <Card title="Persönlichkeitsrecht" icon="user-shield">
    Verletzung des allgemeinen Persönlichkeitsrechts
  </Card>
  <Card title="Datenschutz" icon="shield-halved">
    DSGVO-Verstöße mit Bußgeldern bis 4% des Jahresumsatzes
  </Card>
  <Card title="Urheber-/Leistungsschutzrecht" icon="copyright">
    Verletzung von Künstler- und Sprecherrechten
  </Card>
  <Card title="Strafrecht" icon="gavel">
    Mögliche strafrechtliche Konsequenzen bei Betrug
  </Card>
</CardGroup>

#### Wirtschaftliche Risiken

<Steps>
  <Step title="Rechtstreitigkeiten">
    Hohe Kosten durch Gerichtsverfahren und Anwaltshonorare
  </Step>
  <Step title="Schadensersatz">
    Entschädigungszahlungen für materielle und immaterielle Schäden
  </Step>
  <Step title="Bußgelder">
    DSGVO-Sanktionen durch Datenschutzbehörden
  </Step>
  <Step title="Reputationsschäden">
    Vertrauensverlust bei Kunden und Geschäftspartnern
  </Step>
</Steps>

#### Persönliche Risiken für Betroffene

<Accordion title="Identitätsdiebstahl">
Missbrauch der geklonten Stimme für kriminelle Zwecke wie CEO-Fraud oder Erpressung.
</Accordion>

<Accordion title="Rufschädigung">
Veröffentlichung kompromittierender gefälschter Aussagen, die der Person zugeschrieben werden.
</Accordion>

<Accordion title="Desinformation">
Nutzung der Stimme zur Verbreitung von Falschinformationen oder manipulativen Inhalten.
</Accordion>

<Accordion title="Wirtschaftliche Schäden">
Verlust von Einnahmen durch unautorisierte kommerzielle Nutzung der Stimme.
</Accordion>

---

## Steuerliche Aspekte

### Einkommensteuer

<Info>
**Lizenzgebühren** unterliegen in Deutschland, Österreich und der Schweiz grundsätzlich der **Einkommensteuer**.
</Info>

#### Grenzüberschreitende Lizenzen

Bei internationalen Lizenzverträgen können **Quellensteuern** anfallen:

<CardGroup cols={3}>
  <Card title="Deutschland" icon="flag">
    5-10% Quellensteuer je nach Doppelbesteuerungsabkommen
  </Card>
  <Card title="Österreich" icon="flag">
    0-10% abhängig von Vertragsland
  </Card>
  <Card title="Schweiz" icon="flag">
    0-8% nach Doppelbesteuerungsabkommen
  </Card>
</CardGroup>

### Empfehlungen

<Steps>
  <Step title="Vertragliche Klarstellung">
    Eindeutige Regelung der Steuerlast im Lizenzvertrag
  </Step>
  <Step title="Steuerberatung">
    Professionelle Beratung bei größeren Lizenzprojekten
  </Step>
  <Step title="Compliance">
    Einhaltung aller steuerlichen Meldepflichten
  </Step>
</Steps>

---

<CardGroup cols={2}>
  <Card title="Zurück zur Übersicht" icon="arrow-left" href="/de/legal/voice-cloning-legal-information">
    Hauptdokument: Rechtliche Informationen
  </Card>
  <Card title="Compliance-Leitfaden" icon="clipboard-check" href="/de/legal/voice-cloning-compliance">
    Praktische Umsetzung und Checklisten
  </Card>
</CardGroup>

<Tip>
Passende Seiten: [Data Retention](/de/legal/data-retention) und [Voice Cloning Legal Information](/de/legal/voice-cloning-legal-information) und [Voice Cloning Compliance](/de/legal/voice-cloning-compliance).
</Tip>
