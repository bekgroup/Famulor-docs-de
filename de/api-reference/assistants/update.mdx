...
Dieser Endpunkt ermöglicht es dir, die Konfiguration eines bestehenden KI-Assistenten zu aktualisieren. Alle Felder sind optional – es werden nur die angegebenen Felder aktualisiert. Für ausführliche Feldbeschreibungen und Optionen siehe [Assistent erstellen](/de/api-reference/assistants/create).
...
<ParamField body="voice_id" type="integer">
  Die zu verwendende Stimmen-ID für den Assistenten. Verwende den Endpunkt [Stimmen abrufen](/de/api-reference/assistants/voices) mit dem Parameter `mode`, um kompatible Stimmen für deinen Engine-Modus zu erhalten.
</ParamField>

<ParamField body="language_id" type="integer">
  Die Sprach-ID für den Assistenten. Verwende den Endpunkt [Sprachen abrufen](/de/api-reference/assistants/languages), um verfügbare Sprachen zu erhalten.
</ParamField>
...
<ParamField body="llm_model_id" type="integer">
  ...
  Verwende den Endpunkt [Modelle abrufen](/de/api-reference/assistants/models), um verfügbare Modelle zu erhalten.
</ParamField>

<ParamField body="multimodal_model_id" type="integer">
  Die multimodale Modell-ID. Gilt nur für die Modi `multimodal` und `dualplex`.

  Verwende den Endpunkt [Modelle abrufen](/de/api-reference/assistants/models), um verfügbare multimodale Modelle zu erhalten.
</ParamField>
...
<ParamField body="secondary_language_ids" type="integer[]">
  Array mit zusätzlichen Sprach-IDs, die der Assistent sprechen kann. Ersetzt bestehende Sekundärsprachen. Übergib ein leeres Array `[]`, um alle Sekundärsprachen zu entfernen.
...
<ParamField body="knowledgebase_mode" type="string">
  ...
  * `function_call` - KI ruft eine Funktion zur Suche auf (erforderlich für multimodal/dualplex)
...
<ParamField body="folder_id" type="integer">
  ID eines [Ordners](/de/api-reference/folders/create), in den dieser Assistent verschoben werden soll. Muss zu deinem Konto gehören. Auf `null` setzen, um ihn unkategorisiert zu machen. Weglassen, um den Ordner unverändert zu lassen.
</ParamField>

<ParamField body="label_ids" type="integer[]">
  Array von [Label](/de/api-reference/labels/create)-IDs für diesen Assistenten. Jedes Label muss zu deinem Konto gehören. Wenn angegeben, **ersetzt** dies die aktuellen Labels des Assistenten. Weglassen, um Labels unverändert zu lassen; `[]` senden, um alle Labels zu entfernen.
...
<ParamField body="phone_number_id" type="integer">
  Die ID einer zuzuweisenden Telefonnummer. Auf `null` setzen zum Aufheben der Zuweisung. Muss zu deinem Konto gehören.
...
<ParamField body="tool_ids" type="integer[]">
  Array von benutzerdefinierten Mid-Call Tool-IDs zur Synchronisierung. **Ersetzt alle bestehenden Tool-Zuweisungen.** Übergib ein leeres Array `[]`, um alle Tools zu entfernen. Jedes Tool muss zu deinem Konto gehören.
...
<ParamField body="tools" type="array">
  Array von integrierten Tools. **Ersetzt alle bestehenden integrierten Tools.** Übergib ein leeres Array `[]`, um alle Tools zu entfernen.
  ...
  <Expandable title="Tool-Typen">
    ...
    * `custom`: Wenn true, kann die KI die Weiterleitungsnummer dynamisch bestimmen
    * `timezone`: Zeitzone für die Erreichbarkeit der Weiterleitung
    * `warm_transfer`: Nachricht an den Kunden vor der Weiterleitung senden (Standard: `false`)
    ...
    * `supervisor_phone` (erforderlich): Telefonnummer für den Warm-Transfer (z. B. „+14155552001“). Bei aktiviertem `custom_sip` stattdessen SIP-Adresse oder interne Durchwahl.
    * `outbound_phone_id` (erforderlich): ID der Telefonnummer, mit der der Supervisor angerufen wird. Siehe [Telefonnummern abrufen](/de/api-reference/assistants/phone-numbers).
    * `description` (erforderlich): Wann transferieren — beschreibt, wann die KI den Warm-Transfer auslösen soll (z. B. „An einen menschlichen Supervisor weiterleiten, wenn der Kunde mit einer echten Person sprechen möchte.“)
    * `custom_sip`: Eigene SIP-Adresse oder interne Durchwahl statt Telefonnummer (Standard: `false`)
    * `caller_id_mode`: Welche Nummer der Supervisor beim Anruf sieht. Optionen: `outbound_number` (Standard — Outbound-Nummer), `customer_number` (Anrufernummer), `custom` (eigene Nummer)
    * `custom_caller_id`: Eigene Nummer für den Supervisor. Nur bei `caller_id_mode: custom`.
    * `hold_music`: Audio während des Wartens. Optionen: `hold_music` (Standard — Standard-Wartemusik), `none` (Stille)
    * `hold_music_volume`: Lautstärke der Wartemusik, 0–100 (Standard: `80`)
    * `hold_message`: Ansage an den Anrufer vor dem Halten (Standard: „Bitte warten Sie, während ich Sie mit einem Supervisor verbinde.“)
    * `summary_instructions`: Anweisung, wie die KI den Supervisor briefen soll (Standard: „Stellen Sie das Gespräch aus Ihrer Sicht vor:\n- WER ruft an (Name, Firma falls erwähnt)\n- WARUM sie angerufen haben (Ziel oder Problem)\n- WARUM jetzt ein Mensch nötig ist\n\nHalten Sie es kurz (2-3 Sätze).“)
    * `briefing_initial_message`: Erste Nachricht der KI an den Supervisor (Standard: „Hallo! Ich habe einen Anrufer in der Leitung, der Ihre Hilfe braucht. Darf ich Sie kurz einweisen?“)
    * `connected_message`: Ansage an den Anrufer nach Verbindung mit dem Supervisor (Standard: „Sie sind jetzt mit einem Supervisor verbunden. Ich überlasse Sie ihm.“)
    ...
    * `timeout`: Sekunden bis zur Eingabe, 1–30 (Standard: `5`)
    * `stop_key`: Taste zum Beenden. Optionen: `#` (Standard), `*`

    **calendar\_integration** - Termine über Cal.com planen

    * `calcom_api_key` (erforderlich): Dein Cal.com API-Schlüssel
    ...
    * `calcom_endpoint`: Cal.com API-Region. Optionen: `us` (Standard — `https://api.cal.com`), `eu` (`https://api.cal.eu`), `custom` (nutzt `calcom_custom_endpoint`)
    * `calcom_custom_endpoint`: Eigene Cal.com API-Basis-URL. Nur bei `calcom_endpoint: custom` (z. B. `https://my-calcom-instance.com`).
    * `calcom_booking_fields`: Array benutzerdefinierter Buchungsfelder. Pro Feld: `slug` (erforderlich), `type` (erforderlich, z. B. „text“, „email“, „phone“, „select“), `label` (erforderlich), `required` (Standard: `false`), `options` (Optionen für Select-Felder)
    * `description`: Wann Terminbuchung angeboten werden soll

    **assistant\_transfer** - Gespräch während des Anrufs an einen anderen KI-Assistenten übergeben (tauscht Stimme, LLM und STT zum Ziel-Assistenten)

    * `assistant_id` (erforderlich): ID des Ziel-Assistenten. Muss zu deinem Konto gehören und darf nicht der aktualisierte Assistent selbst sein (kein Self-Transfer).
    * `description`: Wann transferiert werden soll (max. 500 Zeichen, Standard: „Übergeben Sie das Gespräch an diesen Assistenten, wenn es passend ist.“)
    * `message_before_transfer`: Optionale Nachricht, die die KI vor dem Wechsel spricht (max. 500 Zeichen).
    * `speak_transfer_greeting`: Wenn true, spricht der Ziel-Assistent nach dem Transfer seine konfigurierte Erstnachricht (Standard: `true`)
  </Expandable>
  ...
  "tools": [
    {
      "type": "dtmf_input",
      "description": "IVR bei Bedarf navigieren"
    },
    {
  ...
  <Note>
    In der [Assistenten-Liste](/de/api-reference/assistants/list) wird jedes konfigurierte Tool als `{ "type": "...", "data": { ... } }` zurückgegeben — dieselben Feldnamen wie im Request, aber unter `data` verschachtelt (nicht top-level).
  </Note>
</ParamField>
...
<ParamField body="synthesizer_provider_id" type="integer">
  Benutzerdefinierte TTS-Provider-ID. Auf `null` setzen, um den Sprachstandard zu verwenden. Siehe [Synthesizer-Provider abrufen](/de/api-reference/assistants/synthesizer-providers).
</ParamField>

<ParamField body="transcriber_provider_id" type="integer">
  Benutzerdefinierte STT-Provider-ID. Nur Pipeline-Modus. Auf `null` setzen, um den Sprachstandard zu verwenden. Siehe [Transcriber-Provider abrufen](/de/api-reference/assistants/transcriber-providers).
</ParamField>
...
* Nur die Felder, die du im Request Body angibst, werden aktualisiert
* Um optionale Felder zu entfernen, setze diese explizit auf `null`
* Um Arrays zu leeren (Tools, Sekundärsprachen), übergib ein leeres Array `[]`
* Der Assistent muss dem authentifizierten Benutzer gehören
* Verwende den Endpunkt [Stimmen abrufen](/de/api-reference/assistants/voices) mit dem Parameter `mode`, um kompatible Stimmen zu erhalten
