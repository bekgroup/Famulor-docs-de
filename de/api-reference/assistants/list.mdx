...
<ResponseField name="data" type="array">
  ...
  <Expandable title="Assistenten-Eigenschaften">
    ...
    <ResponseField name="tools" type="array">
      Array der konfigurierten Built-in-Tools des Assistenten. Jedes Element hat `type` (Tool-Kennung) und `data` (tool-spezifische Einstellungen). Dieses Response-Format unterscheidet sich vom flachen Objekt in [Create](/de/api-reference/assistants/create)- und [Update](/de/api-reference/assistants/update)-Requests.

      <Expandable title="Response-Element">
        * `type` — z. B. `end_call`, `call_transfer`, `warm_call_transfer`, `dtmf_input`, `collect_keypad`, `calendar_integration`, `assistant_transfer`
        * `data` — Objekt mit den Konfigurationsfeldern (gleiche Namen wie im Request-Body, aber unter `data` verschachtelt)
      </Expandable>
    </ResponseField>
      ...
      "tools": [
        {
          "type": "end_call",
          "data": {
            "description": "Anruf beenden, nachdem Sie sich vom Kunden verabschiedet haben."
          }
        },
        {
