...
Dieser Endpunkt gibt eine Cursor-paginierte Liste von Konversationen zurück, die den Assistenten des authentifizierten Benutzers gehören. Verwende dies, um den Konversationsverlauf anzuzeigen, nach Typ zu filtern oder eine Integration mit deinem CRM durchzuführen.

<Note>
  Dieser Endpunkt verwendet Cursor-basierte Paginierung für eine bessere Performance bei großen Datensätzen. Verwende `next_cursor` und `prev_cursor`, um zwischen den Seiten zu navigieren.
</Note>
...
<ParamField query="external_identifier" type="string" optional>
  Filtert Konversationen nach einer externen Kennung. Nützlich, um Konversationen zu finden, die mit Datensätzen in deinem externen System verknüpft sind.
</ParamField>
...
<ParamField query="cursor" type="string" optional>
  Cursor für die Paginierung. Verwende `next_cursor` oder `prev_cursor` aus einer vorherigen Antwort.
</ParamField>
...
<ResponseField name="data" type="array">
  <Expandable title="Eigenschaften">
    ...
    <ResponseField name="external_identifier" type="string">
      Die Kennung deines externen Systems für diese Konversation. Nur bei Konversationen vom Typ `api` vorhanden, wenn eine externe Kennung gesetzt wurde.
    </ResponseField>
...
<ResponseField name="next_cursor" type="string">
  Cursor zum Abrufen der nächsten Seite mit Ergebnissen. Übergib dies als Parameter `cursor` in deiner nächsten Anfrage. `null`, wenn keine weiteren Ergebnisse vorliegen.
</ResponseField>
...
| `external_identifier`   | Nach deiner externen ID filtern     | Konversationen mit deinen CRM-Datensätzen verknüpfen   |

## WhatsApp-Konversationsdetails

Für WhatsApp-Konversationen enthält die Antwort zusätzliche Felder:

* **whatsapp\_sender**: Die Business-WhatsApp-Nummer, die das Gespräch geführt hat (Name und Telefonnummer deines WhatsApp-Absenders)
* **customer**: Der Kunde, der das Gespräch initiiert oder empfangen hat (dessen Name und Telefonnummer)

Diese Felder sind nur bei Konversationen vom Typ `whatsapp` vorhanden und helfen dir, die beteiligten Parteien zu identifizieren, wenn du eine Integration mit deinen CRM- oder Support-Systemen vornimmst.

## API-Konversationsdetails

Für über die API erstellte Konversationen kannst du beim Erstellen der Konversation einen `external_identifier` festlegen. Diese Kennung wird in der Antwort zurückgegeben und kann für Folgendes verwendet werden:

* Verknüpfung von Konversationen mit deinen CRM-Leads oder -Kontakten
* Nachverfolgung von Konversationen über deine internen Systeme hinweg
* Filterung von Konversationen nach deiner externen Referenz

## Anwendungsfälle

* **Analyse-Dashboard**: Anzeige von Konversationsmetriken und Trends
* **CRM-Integration**: Synchronisierung von Konversationsdaten mit deiner Kundendatenbank unter Verwendung von `external_identifier`
...
* **Abrechnungsprüfung**: Verfolgung der Konversationskosten in deinem Unternehmen
