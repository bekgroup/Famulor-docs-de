...
<ParamField body="assistant_id" type="integer" required>
  Die ID des Assistenten, der für die Antwortgenerierung verwendet werden soll. Muss zu deinem Konto gehören.
</ParamField>
...
<ParamField body="customer_identifier" type="string" required>
  ...
  **Wichtig**: Verwende immer dasselbe Format für denselben Kunden, damit der Kontext korrekt zugeordnet wird.
</ParamField>
...
<ResponseField name="conversation_id" type="string">
  Die UUID der Konversation. Verwende diese, um die Konversation später zu verfolgen oder darauf zu verweisen.
</ResponseField>
...
<ResponseField name="error_code" type="string">
  ...
  - `ASSISTANT_NOT_FOUND` - Die Assistenten-ID ist ungültig oder gehört nicht zu deinem Konto
  - `INSUFFICIENT_BALANCE` - Dein Kontoguthaben ist zu niedrig, um die Nachricht zu verarbeiten
</ResponseField>

### Anwendungsfälle

#### Multi-Channel KI-Antworten
Nutze diesen Endpunkt, um KI-Antworten zu jeder Messaging-Plattform hinzuzufügen:
1. Empfange eine Nachricht von WhatsApp, Facebook, SMS oder einem anderen Kanal
2. Rufe diesen Endpunkt mit der Nachricht und der Kundenkennung auf
3. Sende die KI-Antwort über den ursprünglichen Kanal zurück

#### CRM-Integration
Integriere KI-Antworten in dein CRM oder Helpdesk-System:
- Verwende die CRM-Kontakt-ID als `customer_identifier`
- Übergib Kundendaten als Variablen für personalisierte Antworten
- Die Konversation bleibt über Sitzungen hinweg erhalten, wenn dieselbe Kennung verwendet wird

#### Eigene Chat-Oberflächen
Erstelle deine eigene Chat-Oberfläche, die von deinem Famulor-Assistenten angetrieben wird:
- Generiere eine eindeutige Kennung für jede Benutzersitzung
- Sende Nachrichten über diesen Endpunkt
- Zeige die KI-Antworten in deiner Oberfläche an
...
- **Konsistente Kennungen verwenden**: Verwende immer dasselbe Format für Kundenkennungen (z.B. immer E.164 für Telefonnummern)
- **Relevanten Kontext übergeben**: Nutze das `variables`-Feld, um Kundendaten bereitzustellen, die der KI helfen, Antworten zu personalisieren
- **Rate Limits handhaben**: Implementiere Wiederholungslogik mit exponentiellem Backoff für rate-limitierte Anfragen
- **Konversations-IDs speichern**: Speichere die zurückgegebene `conversation_id` für spätere Referenz oder Fehlerbehebung
- **Kosten überwachen**: Verfolge die Nutzung, um Kosten zu verwalten, insbesondere bei Integrationen mit hohem Volumen
