...
Der Post-Call Webhook wird automatisch an deine angegebene Webhook-URL gesendet, nachdem jeder Anruf abgeschlossen wurde. Dieser Webhook enthält Anrufdetails, extrahierte Variablen, Transkript und optional die Aufzeichnungs-URL.

## Webhook-Konfiguration

Um Post-Call Webhooks zu aktivieren:

1. Konfiguriere die Webhook-URL deines Assistenten in den Assistenten-Einstellungen
2. Aktiviere "Post call webhook" in der Assistenten-Konfiguration
3. Definiere Post-Call-Variablen, die die KI extrahieren soll
...
<ResponseField name="extracted_variables" type="object">
  Von der KI extrahierte Variablen basierend auf deiner Post-Call-Schema-Konfiguration
...
Du kannst die folgenden Optionen für deinen Post-Call Webhook konfigurieren:
...
- Das Feld `recording_url` ist nur enthalten, wenn du "Include recording in webhook" in deinen Assistenten-Einstellungen aktiviert hast
- Das `lead`-Objekt ist nur bei Anrufen enthalten, die Teil einer Kampagne sind
- Das `extracted_variables`-Objekt enthält die Variablen, die du in der Post-Call-Schema-Konfiguration deines Assistenten definiert hast
