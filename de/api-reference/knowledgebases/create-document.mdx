...
<Note>
  Die Dokumentverarbeitung erfolgt asynchron. Rufen Sie den [Dokument abrufen](/de/api-reference/knowledgebases/get-document) Endpunkt ab, um zu prüfen, wann die Verarbeitung abgeschlossen ist.
</Note>
...
const response = await fetch(
  "https://app.famulor.de/api/user/knowledgebases/1/documents",
  {
    method: "POST",
    headers: {
...
```javascript JavaScript (PDF Upload)
const formData = new FormData();
formData.append("name", "Produkthandbuch");
formData.append("description", "Benutzerhandbuch für unser Produkt");
formData.append("type", "pdf");
formData.append("file", fileInput.files[0]); // fileInput ist ein <input type="file">
