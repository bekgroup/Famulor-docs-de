...
Dieses Template ermöglicht es deinem KI-Assistenten, während eines Gesprächs automatisch Kontaktdaten aus HubSpot abzurufen. Sobald ein Kunde seine E-Mail-Adresse nennt, werden die relevanten Informationen sofort verfügbar gemacht.
...
<Steps>
  <Step title="HubSpot Dashboard öffnen">
    - Melde dich in deinem HubSpot-Account an
    - Navigiere zu "Einstellungen" (⚙️)
  </Step>
  
  <Step title="API-Key generieren">
    - Geh zu "Integrationen" → "Private Apps"
    - Klicke auf "Create a private app"
    - Vergib einen aussagekräftigen Namen (z.B. "Famulor Mid-call Actions")
  </Step>
  ...
  <Step title="API-Key sichern">
    ...
    - Speichere ihn sicher (wird für die Tool-Konfiguration benötigt)
  </Step>
</Steps>

### 2. Mid-call Action konfigurieren
...
<AccordionGroup>
  ...
  <Accordion title="Header-Konfiguration">
    **Authentifizierung**:
    ...
    <Warning>
    **Sicherheitshinweis**: Verwende Umgebungsvariablen für API-Keys. Niemals API-Keys direkt im Code speichern!
    </Warning>
  </Accordion>
...
Nachdem du das HubSpot Kontakt-Abruf-Tool implementiert hast, kannst du weitere HubSpot-Integrationen hinzufügen:

- [Deal-Management Tool](/de/automation-platform/mid-call-tools/integration-templates/hubspot-deal-management)
- [Lead-Erstellung Tool](/de/automation-platform/mid-call-tools/integration-templates/hubspot-lead-erstellung)
- [Webhook-Integration](/de/automation-platform/mid-call-tools/integration-templates/webhook-automation) für komplexere Workflows
</Card>

<Warning>
**Wichtiger Hinweis**: Teste die Integration zunächst in einer Entwicklungsumgebung, bevor du sie in der Produktion einsetzt. Überwache die Performance kontinuierlich und implementiere entsprechende Fallback-Mechanismen.
</Warning>
