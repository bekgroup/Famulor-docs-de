...
description: 'Verbinde dein Easybell-Telefonsystem mit Famulor über SIP-Trunking oder die Cloud Telefonanlage'
---
...
<Note>
  Mit Famulor kannst du deine **Easybell**-Telefonie mit einem KI-Assistenten verbinden. Es gibt **zwei Wege** – wähle unten deinen Weg und folge der passenden Schritt-für-Schritt-Anleitung.
</Note>
...
Anrufe laufen über Easybell zu Famulor und werden dort deinem KI-Assistenten zugeordnet. Ablauf eines **eingehenden Anrufs**:
...
sequenceDiagram
    ...
    K->>E: Anruf an deine Easybell-Nummer
...
Nutze **nur** den Easybell SIP-Trunk (ohne Cloud Telefonanlage). Geh die Schritte der Reihe nach durch.

### Schritt 1: FQDN-Feature bei Easybell anfordern

Das FQDN-Feature ist im Easybell-Kundenportal nicht buchbar. Sende eine E-Mail an **support@easybell.de** mit deiner **Easybell-Kundennummer** und bitte um Freischaltung des SIP Trunking FQDN Feature für Famulor.
...
4. **Eingehende Telefonie** einstellen:
   ...
   - **„Unsere SIP-Adresse“** notieren – diese brauchst du in Schritt 3 für Easybell.
...
**Beispiel:** `XXXXX.sip.livekit.cloud` (Deine Adresse steht in Famulor unter „Unsere SIP-Adresse“.)

UDP, TCP und TLS werden unterstützt. Für das **Rufnummernformat** E.164 **mit** führendem „+“ wählen.

#### Authentifizierung per FQDN im Modus „Trunk“

Wenn du bei Easybell im Modus **„Trunk“** arbeitest, richte die Authentifizierung wie folgt ein:

- **Authentifizierung durch:** `vollqualifizierter Domainname (FQDN)` auswählen.  
- **FQDN:** exakt den Wert eintragen, der in Famulor unter **„Unsere SIP-Adresse“** angezeigt wird (ohne `sip:` davor).  
- **SRTP nutzen:** **deaktiviert** lassen (Häkchen darf **nicht** gesetzt sein).  

Die Konfiguration sieht dann z. B. wie im folgenden Screenshot aus:

<img src="/images/provisioning/sip-ai/trusteip-FQDN-de.png" alt="Easybell Trusted IP Einstellungen mit Authentifizierung per FQDN (SRTP deaktiviert)" />
...
- Eingehenden Anruf an deine Easybell-Nummer testen.  
...
Du nutzt die **Easybell Cloud Telefonanlage Pro**. FQDN ist hier bereits enthalten – keine E-Mail an Easybell nötig. Zuerst in Famulor einrichten (wie beim SIP-Trunk), dann in der Cloud Telefonanlage.
...
<AccordionGroup>
  <Accordion title="FQDN-Feature nicht verfügbar (SIP-Trunk)" icon="envelope">
    Das SIP Trunking FQDN Feature muss bei Easybell angefragt werden. Sende eine E-Mail an **support@easybell.de** mit deiner **Easybell-Kundennummer** und der Bitte um Freischaltung für Famulor. Die Aktivierung kann einige Tage dauern.
  </Accordion>

  <Accordion title="Anrufe kommen nicht an" icon="phone-slash">
    Prüfe: **Erlaubte IP-Adressen** in Famulor unter „Eingehende Telefonie“ (je nach Weg SIP-Trunk- oder Cloud-IPs eintragen). Stelle in Easybell das Rufnummernformat **E.164 mit führendem „+“** ein.
  </Accordion>

  <Accordion title="Falsche oder unbekannte Domain" icon="server">
    Trage die **genaue SIP-Adresse** aus Famulor ein („Unsere SIP-Adresse“ unter Telefonnummern → SIP-Trunk integrieren). **Ohne** `sip:` davor, **ohne** Leerzeichen – nur den angezeigten Wert übernehmen (z. B. `… .sip.livekit.cloud`).
  </Accordion>

  <Accordion title="Weiterleitung an Nebenstelle funktioniert nicht (Cloud Telefonanlage)" icon="phone-flip">
...
<Tip>
  Unser Support unterstützt dich bei der Einrichtung: [support@famulor.io](mailto:support@famulor.io). Für Fragen zu FQDN/Produkten: [support@easybell.de](mailto:support@easybell.de).
</Tip>
