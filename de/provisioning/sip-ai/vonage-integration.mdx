...
## Anrufweiterleitung an einen Menschen (Warmweiterleitung)

Famulor kann ein laufendes Gespräch an einen echten Ansprechpartner übergeben. **Bei Vonage nutzt du dafür ausschließlich die Warmweiterleitung** – die Kaltweiterleitung funktioniert über den Vonage-Trunk nicht (siehe Hinweis). Die allgemeine Einrichtung findest du unter [Anrufsweiterleitung](/de/platform/anrufsweiterleitung).
...
### Weiterleitungsziel: Telefonnummer oder Custom SIP

Das Ziel der Warmweiterleitung gibst du auf eine von zwei Arten an:

- **Supervisor-Telefonnummer:** Die Rufnummer des Ansprechpartners (z. B. `+49 1512 3456789`).
- **(Advanced) Custom SIP/Extension:** Aktiviere diese Option, um statt einer Telefonnummer eine **eigene SIP-Adresse oder interne Nebenstelle** anzugeben (Feld **Custom SIP transfer**, z. B. `sip:number@sip-server`).

<img
  src="/images/vonage/famulor-warm-transfer.png"
  alt="Famulor: Warmweiterleitung mit Supervisor-Telefonnummer (Custom SIP/Extension deaktiviert)"
/>

<img
  src="/images/vonage/famulor-warm-transfer-custom-sip.png"
  alt="Famulor: Warmweiterleitung mit aktiviertem „(Advanced) Custom SIP/Extension“ und Feld „Custom SIP transfer“"
/>

### Einstellungen der Warmweiterleitung

| Feld | Bedeutung |
| --- | --- |
| **Supervisor phone number** / **Custom SIP transfer** | Ziel des Transfers: Telefonnummer **oder** – bei aktiviertem **Custom SIP/Extension** – SIP-Adresse bzw. Nebenstelle |
| **Outbound phone number** | Nummer, über die der Supervisor angerufen wird (benötigt einen ausgehenden Trunk) |
| **Caller ID shown to supervisor** | Welche Nummer der Supervisor beim Anruf sieht (z. B. die ausgehende Nummer) |
| **Hold music** / **Hold music volume** | Wartemusik für den Anrufer und Lautstärke (0–100) |
| **When to transfer** | Beschreibung, wann die KI die Warmweiterleitung auslösen soll |
| **Hold message** | Was die KI dem Anrufer sagt, bevor er in die Warteschleife kommt |
| **Summary instructions** | Wie die KI den Supervisor briefen soll (wer ruft an, warum, warum ein Mensch nötig ist) |
| **Briefing initial message** | Erster Satz, den der Supervisor hört |
| **Connected message** | Was der Anrufer hört, sobald der Supervisor verbunden ist |

<Note>
  In **Summary instructions**, **Briefing initial message** und ähnlichen Feldern kannst du Variablen verwenden: `{customer_phone}` (Rufnummer des Anrufers) sowie Lead-Variablen wie `{name}` oder `{email}`.
</Note>
