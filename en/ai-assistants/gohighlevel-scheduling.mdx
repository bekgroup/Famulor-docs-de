...
1. In your AI assistant settings, navigate to the **Tools** section
...
- You'll be redirected to GoHighLevel's authorization page
- Choose your account from the available options
- Authorize the application to access your calendar data
- You'll be redirected back to your assistant settings
...
- Make a test call to your AI assistant
- Request to schedule an appointment during the conversation
- Verify the booking appears in your GoHighLevel calendar
- Check that confirmation details are sent appropriately
- Test different scenarios (available/unavailable times)
...
Setup options:
- **Define email in input variables**: Add email as an input variable in your assistant settings
- **Import leads with email**: When importing leads, ensure email addresses are included
- **AI collection during call**: Configure your assistant to ask for email during the conversation if not available
...
**Appointment Duration Setup**: Tell your AI assistant about appointment lengths in the system prompt:
...
- **Steps**: Go to assistant settings → Appointment Scheduling → Disconnect → Connect again → Select assistant → Save
