...
## Call transfer to a human (warm transfer)

Famulor can hand off an ongoing call to a real person. **For Vonage, use the warm transfer only** – cold transfer does not work over the Vonage trunk (see the note below). For the general setup, see [Assistant call forwarding](/en/platform/assistant-call-forwarding).
...
### Transfer target: phone number or custom SIP

You specify the warm transfer target in one of two ways:

- **Supervisor phone number:** the contact's phone number (e.g. `+49 1512 3456789`).
- **(Advanced) Custom SIP/Extension:** enable this to specify a **custom SIP address or internal extension** instead of a phone number (field **Custom SIP transfer**, e.g. `sip:number@sip-server`).

<img
  src="/images/vonage/famulor-warm-transfer.png"
  alt="Famulor: warm transfer with a supervisor phone number (Custom SIP/Extension disabled)"
/>

<img
  src="/images/vonage/famulor-warm-transfer-custom-sip.png"
  alt="Famulor: warm transfer with „(Advanced) Custom SIP/Extension“ enabled and the „Custom SIP transfer“ field"
/>
...
| Field | Meaning |
| --- | --- |
| **Supervisor phone number** / **Custom SIP transfer** | Transfer target: a phone number **or** – with **Custom SIP/Extension** enabled – a SIP address or extension |
| **Outbound phone number** | The number used to dial the supervisor (requires an outbound trunk) |
| **Caller ID shown to supervisor** | Which number the supervisor sees on the call (e.g. the outbound number) |
| **Hold music** / **Hold music volume** | Hold music played to the caller and its volume (0–100) |
| **When to transfer** | Describes when the AI should start the warm transfer |
| **Hold message** | What the AI says to the caller before placing them on hold |
| **Summary instructions** | How the AI should brief the supervisor (who is calling, why, why a human is needed) |
| **Briefing initial message** | The first message the supervisor hears |
| **Connected message** | What the caller hears once the supervisor is connected |

<Note>
  In **Summary instructions**, **Briefing initial message** and similar fields you can use variables: `{customer_phone}` (the caller's number) and lead variables such as `{name}` or `{email}`.
</Note>
