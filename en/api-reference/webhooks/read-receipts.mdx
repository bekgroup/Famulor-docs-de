...
1. Edit your [WhatsApp sender](/en/whatsapp/senders) and open the **Read Receipts Webhook** section
...
Each payload carries the `whatsapp_message_id` returned by the [Send Template](/en/api-reference/whatsapp/send-template) and [Send Free-form](/en/api-reference/whatsapp/send-freeform) endpoints, so you can match every update to the original message.
