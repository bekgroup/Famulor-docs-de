...
description: "Send a WhatsApp message using an approved template via Famulor"
---

Send a WhatsApp message using a pre-approved Meta template through your Famulor WhatsApp sender. Template messages are required when initiating a conversation with a user for the first time or when messaging outside the 24-hour messaging window.
...
<ParamField body="template_id" type="integer" required>
  The ID of the message template to use (obtained from the [Get Templates](/en/api-reference/whatsapp/get-templates) endpoint)
</ParamField>
...
* Message costs are automatically deducted from your Famulor account balance (credits for tenant users, minutes for direct users).
* After sending a template message, a 24-hour messaging window opens. During this window, you can send [freeform messages](/en/api-reference/whatsapp/send-freeform) without needing a template.
