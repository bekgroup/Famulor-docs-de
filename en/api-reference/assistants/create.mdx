...
<ParamField body="folder_id" type="integer">
  ID of a [folder](/en/api-reference/folders/create) to place this assistant in. Must belong to your account. Send `null` to leave the assistant uncategorized.
</ParamField>

<ParamField body="label_ids" type="integer[]">
  Array of [label](/en/api-reference/labels/create) IDs to apply to this assistant. Each label must belong to your account.
...
<ParamField body="tools" type="array">
  Array of built-in tools. Each tool has a `type` field and tool-specific fields. On [Update Assistant](/en/api-reference/assistants/update), this **replaces all existing built-in tools** — pass an empty array `[]` to remove all tools.
  ...
  <Expandable title="Tool Types">
    ...
    * `outbound_phone_id` (required): ID of the phone number used to dial the supervisor. See [Retrieve Phone Numbers](/en/api-reference/assistants/phone-numbers).
