...
<ParamField body="folder_id" type="integer">
  ID of a [folder](/en/api-reference/folders/create) to move this assistant into. Must belong to your account. Set to `null` to make it uncategorized. Omit to leave the folder unchanged.
</ParamField>

<ParamField body="label_ids" type="integer[]">
  Array of [label](/en/api-reference/labels/create) IDs for this assistant. Each label must belong to your account. When provided, this **replaces** the assistant's current labels. Omit to leave labels unchanged; send `[]` to remove all labels.
...
<ParamField body="tools" type="array">
  ...
  <Expandable title="Tool Types">
    ...
    * `outbound_phone_id` (required): ID of the phone number used to dial the supervisor. See [Get Phone Numbers](/en/api-reference/assistants/phone-numbers).
  ...
  <Note>
    When you [list assistants](/en/api-reference/assistants/list), each configured tool is returned as `{ "type": "...", "data": { ... } }` with the same field names nested under `data` (not top-level).
  </Note>
</ParamField>
