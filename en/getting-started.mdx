---
title: "Getting Started"
description: "Get started with Famulor: create your first AI voice agent, connect a phone number and launch inbound or outbound calls in minutes."
keywords: ["getting started", "Famulor setup", "AI voice agent", "first steps", "AI phone agent", "voice AI onboarding", "Famulor tutorial"]
---

This page documents **Getting Started** for the Famulor documentation set.

## Overview

Use this page as the canonical reference for Getting Started in automation and assistant workflows.

## Typical Use Cases

- Standardized implementation in production workflows
- Reusable setup across multiple assistants or integrations
- Faster onboarding for operations and engineering teams

## Setup Guide

1. Open the related section in the automation or integration area.
2. Configure required inputs and dependencies.
3. Validate behavior in a staging environment before production rollout.

## Example Flow

```json Example
{
  "name": "Getting Started",
  "environment": "production",
  "status": "enabled"
}
```

## Best Practices

- Keep naming and mapping conventions consistent.
- Document assumptions next to implementation details.
- Re-test dependent flows after changes.

## See Also

- [No-Code Automation Overview](/en/automation-platform/introduction)
- [Integrations Overview](/en/automation-platform/integrations/overview)
- [Mid-Call Tools](/en/automation-platform/mid-call-tools/was-sind-mid-call-tools)
- [MCP Servers](/en/platform/mcp-servers)

<Tip>
Connect external MCP servers when you need a full toolset (CRM, APIs, knowledge bases) discovered automatically — instead of building each HTTP action by hand. See [MCP Servers](/en/platform/mcp-servers).
</Tip>
